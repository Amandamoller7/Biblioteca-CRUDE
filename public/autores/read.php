...
$nacionalidadeFiltro = $_POST['nacionalidade']??'';
$ano_nacscmientoFiltro = $_POST['ano_nacimento']??'';
    ...
    <ul class="pagination">
        <?php for($i=1; $i<=$total_paginas; $i++): ?>
            <li class="page-item <?= ($i == $pagina) ? 'active' : '' ?>">
                <a class="page-link" 
                   href="?pagina=<?= $i ?>&nome=<?= urlencode($nomeFiltro) ?>&posicao=<?= urlencode($nacionalidadeFiltro) ?>&time=<?= urlencode($ano_nacscmientoFiltro ) ?>">
                    <?= $i ?>
                </a>
            </li>
        <?php endfor; ?>
